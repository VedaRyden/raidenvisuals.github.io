...
soundSalah.volume = 0.1; // Mengecilkan volume suara ERROR menjadi 30% saja
soundGagal.volume = 0.1; // Mengecilkan volume suara GAGAL (Tier 1 & 2) menjadi 40% saja
soundWarn.volume = 0.5;
soundSukses.volume = 0.4; // Mengecilkan volume suara SUKSES (Tier 3 & 4) menjadi 40% saja
...
let ketikanUser = gpuInput.value.replace(/-/g, '').replace(/\s+/g, '').toLowerCase();
    ...
    if (ketikanUser === "" || ketikanUser.length <= 5) {
        ...
        gpuResult.innerHTML = 'Silakan ketik nama GPU nya dulu woilah!<br><img src="img/woylahcik.gif" class="result-sticker">';
    ...
    fetch('tiers.json')
        ...
        .then(data => {
            ...
            if (ditemukan) {
                ...
                if (nomorTier === 1) {
                    ...
                    gpuResult.innerHTML = `❌ Tidak Kompatibel! GPU mu terdeteksi di Tier 1. Sebaiknya ganti HP atau beli HP baru yang spek nya lebih bagus, GPU ini terlalu kuno buat sekarang.<br><img src="img/mkvvgaksupport.jpg" class="result-sticker">`;
                    gpuResult.className = "gpu-result-msg gpu-fail";
                    ...
                    gpuResult.innerHTML = `❌ Tidak Kompatibel! GPU mu terdeteksi di Tier 2. Yang sabar ya semoga bisa dibelikan HP baru Aamiin ^^.<br><img src="img/mkvvgaksupport.jpg" class="result-sticker">`;
                    gpuResult.className = "gpu-result-msg gpu-fail";
                    // KODE BARU: Hanya teks hasil jawaban yang gempa
                    gpuResult.classList.add('efek-gempa');
                    setTimeout(() => {
                    ...
                    gpuResult.innerHTML = `⚠️ Kemungkinan Kompatibel! GPU mu berada di Tier 3. Kalau masih tidak bisa pasti kedepannya bakal bisa karena GPU ini masih memiliki harapan, tetapi performa FPS bisa bervariasi tergantung kondisi perangkat.<br><img src="img/yondaktau.jpg" class="result-sticker">`;
                    ...
                    if (nomorTier === 4) {
                        gpuResult.innerHTML = `✅ SELAMAT! GPU Anda Sudah Kompatibel! GPU mu berada di Tier 4. Vibrant Visuals akan berjalan cukup lancar dan optimal di hpmu!<br><img src="img/cihuy.jpg" class="result-sticker">`;
                    } else {
                        gpuResult.innerHTML = `🌟😮✅ Njir HP Gaming Coeg, GPU mu berada di Tier 5. Vibrant Visuals sudah pasti akan berjalan dengan lancar dan optimal di hp gamingmu!<br><img src="img/cihuy.jpg" class="result-sticker">`;
                    }
                ...
                gpuInput.classList.remove('outline-kuning'); // Hapus kuning karena salah isi
                gpuInput.classList.remove('efek-cahaya-hijau');
                gpuResult.innerHTML = '😐 GPU apa ini woy? tolong ketikan nya diperhatikan yah. Pastikan penulisan nama tipe GPU nya sudah benar (Contoh: Adreno 610).<br><img src="img/woylahcik.gif" class="result-sticker">';
                gpuResult.className = "gpu-result-msg gpu-fail";
                gpuInput.classList.add('efek-gempa');
                setTimeout(() => {
                    gpuInput.classList.remove('efek-gempa');
        ...
        .catch(error => {
            ...
            gpuResult.innerHTML = 'Terjadi gangguan sistem saat membaca database.<br><img src="img/waduh.jpg" class="result-sticker">';
            gpuResult.className = "gpu-result-msg gpu-fail";
            gpuResult.style.display = "block";
        });
}
